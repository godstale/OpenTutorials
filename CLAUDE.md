...
### 강좌 제작 Bundler 프로토콜 수정 규칙
- 강좌 번들 구조, JSON 메타데이터 스펙, 유효성 검증 로직 등 **Bundler 프로토콜과 관련된 모든 변경사항이 발생할 경우, 반드시 `docs/bundler/protocol.md` 및 하위 지침 문서들을 즉시 최신화**해야 합니다. 이를 통해 외부 강좌 제작 프로젝트에서 최신 프로토콜에 맞게 강좌 파일을 빌드할 수 있도록 공조 체계를 상시 유지합니다.
